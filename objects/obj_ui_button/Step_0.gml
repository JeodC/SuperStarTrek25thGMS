...
if (menu_id == "ui.scale") {
  ...
  switch (global.scale) {
  case 0:
    text = lang_get("ui.scale1");
    break; // Stretch to fit
  case 1:
    text = lang_get("ui.scale2");
    break; // Integer scale
  }
}
...
if (menu_id == "ui.subtitles") {
  ...
  case 0:
    text = lang_get("opt.audiomode1");
    break;
  case 1:
    text = lang_get("opt.audiomode2");
    break;
  case 2:
    text = lang_get("opt.audiomode3");
    break;
  }
}
...
if (menu_id == "ui.difficulty") {
  ...
  case 1:
    text = lang_get("opt.diff") + ": " + lang_get("opt.diff1");
    break;
  case 2:
    text = lang_get("opt.diff") + ": " + lang_get("opt.diff2");
    break;
  case 3:
    text = lang_get("opt.diff") + ": " + lang_get("opt.diff3");
    break;
  case 4:
    text = lang_get("opt.diff") + ": " + lang_get("opt.diff4");
    break;
  }
}
...
if (!is_disabled) {
  ...
  if (is_array(buttons) && global.selected_index >= 0 &&
      global.selected_index < array_length(buttons)) {
    is_selected = (buttons[global.selected_index] == id);
  }

  // Check if mouse is over this button
  hovered = point_in_rectangle(mouse_x, mouse_y, x, y, x + sprite_width,
                               y + sprite_height);
