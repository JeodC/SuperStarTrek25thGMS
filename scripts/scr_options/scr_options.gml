...
function reorder_languages() {
    ...
    if (global.lang_selected == "" || !array_contains(global.langs, global.lang_selected)) {
        show_debug_message("Invalid language selected: " + string(global.lang_selected));
        return;
    }
    ...
    if (current_index == -1) {
        show_debug_message("Error: Language " + global.lang_selected + " not in global.langs");
        return;
    }
...
function create_title_buttons() {
    cleanup_buttons();
    var titles = ["ui.newgame", "ui.continue", "ui.options", "ui.credits", "ui.exit"];
...
function create_options_buttons() {
	...
	// Difficulty mode
    var b_diff = instance_create_layer(x_base - 30, y_base, "Overlay", obj_ui_button);
    setup_menu_button(b_diff, "ui.difficulty");
    array_push(global.options_buttons, b_diff);
	
	// Audio mode
    var b_sub = instance_create_layer(x_base - 30, y_base + 20, "Overlay", obj_ui_button);
    ...
    var b_left = instance_create_layer(x_base - 24, y_lang, "Overlay", obj_ui_button);
    setup_menu_button(b_left, "ui.langleft");
    array_push(global.options_buttons, b_left);

	// Right language button
    var b_right = instance_create_layer(x_base + 72, y_lang, "Overlay", obj_ui_button);
	...
	// Apply button
    var b_apply = instance_create_layer(x_base - 48, y_action, "Overlay", obj_ui_button);
    setup_menu_button(b_apply, "ui.apply");
    array_push(global.options_buttons, b_apply);

	// Reset defaults button
    var b_default = instance_create_layer(x_base + 48, y_action, "Overlay", obj_ui_button);
    ...
    if (instance_exists(obj_controller_player)) {
        // Return to Game
        var b_return = instance_create_layer(x_base - 48, y_quit, "Overlay", obj_ui_button);
        setup_menu_button(b_return, "ui.return");
        array_push(global.options_buttons, b_return);

        // Quit to Title
        var b_quit = instance_create_layer(x_base + 48, y_quit, "Overlay", obj_ui_button);
        setup_menu_button(b_quit, "ui.quit");
        array_push(global.options_buttons, b_quit);
    } else {
        // Quit (from title/options)
        var b_quit = instance_create_layer(x_base, y_quit, "Overlay", obj_ui_button);
        setup_menu_button(b_quit, "ui.quit");
        array_push(global.options_buttons, b_quit);
    }
}
            ...
            if (!is_undefined(b) && instance_exists(b)) {
                if (b.label == "ui.subtitles") {
                    switch (global.audio_mode) {
                        case 0: b.text = lang_get("opt.audiomode1"); break;
                        case 1: b.text = lang_get("opt.audiomode2"); break;
                        case 2: b.text = lang_get("opt.audiomode3"); break;
                        default: b.text = lang_get("opt.audiomode1");
                    }
                } else if (b.label == "ui.difficulty") {
                    switch (global.difficulty) {
                        case 1: b.text = lang_get("opt.diff1"); break;
                        case 2: b.text = lang_get("opt.diff2"); break;
                        case 3: b.text = lang_get("opt.diff3"); break;
                        case 4: b.text = lang_get("opt.diff4"); break;
                        default: b.text = lang_get("opt.diff1");
                    }
