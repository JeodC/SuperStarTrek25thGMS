...
function init_game() {
	...
	// Resolve queue -- handles all dialog and actions in sequential order, see handle_queue() in scr_resolve
	global.queue = [];
	global.index = 0;
	global.busy = false;
	...
	// ---------------------------------
	// global.allenemies: Array storing all enemy ship instances generated after galaxy creation
	// global.allbases: Array storing all starbase instances generated after galaxy creation
	global.allenemies = [];
	global.allbases = [];
...
function load_language(lang_code) {
    ...
    if (file_exists(lang_path)) {
		...
		try {
		    ...
		    show_debug_message("Error parsing language file: " + lang_path + ". Falling back to English.");
		    _load_english_fallback();
		}
    } else {
        show_debug_message("Missing language file: " + lang_path + ". Falling back to English.");
        _load_english_fallback();
    }
}
...
function _load_english_fallback() {
    ...
    if (file_exists(fallback_path)) {
		...
		try {
			...
			show_debug_message("Error parsing fallback English file: " + fallback_path);
			global.lang_data = {};
		}
    } else {
        show_debug_message("Error: English fallback file (lang/en.json) not found!");
        global.lang_data = {};
    }
}

/// @description: Scales the window to a multiple of the native 320x200 size while preserving pixel clarity
