...
if (show_text) {
    ...
    if (len == 0) {
        ...
        if (voice_handle == -1) {
            var text_length = 0;
            if (is_struct(current) && variable_struct_exists(current, "line") && is_string(current.line)) {
...
/// @description: Plays voice clip associated with the current dialog line if available and audio mode allows it.
/// @param {struct} current_line: The current dialog line struct expected to have a "voice" field (audio resource).
function play_voice(current_line) {
    if (is_struct(current_line) && variable_struct_exists(current_line, "voice") && 
...
/// @description: Advances the dialog index to the next line, resets voice handle and timers.
/// Sets a small delay to prevent immediate re-skipping.
...
/// Stops any playing voice clip, advances text, and starts the voice clip for the new line if available.
