...
function draw_hover_text() {
	...
	switch (obj_controller_input.hover_state) {
		case HoverState.None:               state_name = ""; break;
		case HoverState.Shields:            state_name = lang_get("hotspot.shields"); break;
		case HoverState.Energy:             state_name = lang_get("hotspot.status"); break;
		case HoverState.DamageStatus:       state_name = lang_get("hotspot.damage"); break;
		case HoverState.ScottStatus:        state_name = lang_get("hotspot.scott"); break;
		case HoverState.WarpSpeed:          state_name = lang_get("hotspot.warp"); break;
		case HoverState.GalacticMap:        state_name = lang_get("hotspot.map"); break;
		case HoverState.ImpulseSpeed:       state_name = lang_get("hotspot.impulse"); break;
		case HoverState.LongRangeSensors:   state_name = lang_get("hotspot.lrs"); break;
		case HoverState.Phasers:            state_name = lang_get("hotspot.phasers"); break;
		case HoverState.Torpedoes:          state_name = lang_get("hotspot.torpedoes"); break;
		case HoverState.MissionStatus:      state_name = lang_get("hotspot.mission"); break;
		case HoverState.DockingProcedures:  state_name = lang_get("hotspot.docking"); break;
		case HoverState.Options:            state_name = lang_get("hotspot.options"); break;
		case HoverState.Help:               state_name = lang_get("hotspot.help"); break;
		default:
			if (obj_controller_input.hover_state >= HoverState.Enemy) {
				state_name = lang_get("hotspot.enemy") + " " +  enemy_info; break;
			}
	}
...
function draw_sector_contents() {
    ...
    if (current_sector.basenum > 0) {
        for (var i = 0; i < array_length(global.allbases); i++) {
            ...
            if (!is_undefined(base) && is_struct(base) && base.sx == global.ent.sx && base.sy == global.ent.sy) {
    ...
    for (var i = 0; i < array_length(global.allenemies); i++) {
        ...
        // Safety check -- if an enemy was destroyed they wouldn't be present to draw
        if (!is_undefined(enemy) && is_struct(enemy)) {
            if (enemy.sx == global.ent.sx && enemy.sy == global.ent.sy && enemy_count > 0) {
                ...
                enemy_count--;
                if (enemy_count == 0) break;
            }
        }
    }
    
	// Draw player (animated or static)
	var pos_x = global.ent.animating_impulse ? global.ent.current_x : global.ent.lx;
	var pos_y = global.ent.animating_impulse ? global.ent.current_y : global.ent.ly;
...
function draw_ship_console(report) {
    ...
    switch (report) {
        case Reports.Damage:
            ...
            draw_text(text_x + max_label_width + padding, text_y, string(data.warp_engines) + "%"); text_y += padding;
            
            label = labels.srs;
            draw_set_color(c_white);
            draw_text(text_x, text_y, label);
            draw_set_color(get_color(data.short_range_sensors));
            draw_text(text_x + max_label_width + padding, text_y, string(data.short_range_sensors) + "%"); text_y += padding;
            
            label = labels.lrs;
            draw_set_color(c_white);
            draw_text(text_x, text_y, label);
            draw_set_color(get_color(data.long_range_sensors));
            draw_text(text_x + max_label_width + padding, text_y, string(data.long_range_sensors) + "%"); text_y += padding;
            
            label = labels.phasers;
            draw_set_color(c_white);
            draw_text(text_x, text_y, label);
            draw_set_color(get_color(data.phaser_controls));
            draw_text(text_x + max_label_width + padding, text_y, string(data.phaser_controls) + "%"); text_y += padding;
            
            label = labels.torpedoes;
            draw_set_color(c_white);
            draw_text(text_x, text_y, label);
            draw_set_color(get_color(data.photon_tubes));
            draw_text(text_x + max_label_width + padding, text_y, string(data.photon_tubes) + "%"); text_y += padding;
            
            label = labels.navigation;
            draw_set_color(c_white);
            draw_text(text_x, text_y, label);
            draw_set_color(get_color(data.navigation_computer));
            draw_text(text_x + max_label_width + padding, text_y, string(data.navigation_computer) + "%"); text_y += padding;
            
            label = labels.shields;
            draw_set_color(c_white);
            draw_text(text_x, text_y, label);
            draw_set_color(get_color(data.shields_controls));
            draw_text(text_x + max_label_width + padding, text_y, string(data.shields_controls) + "%");
            break;
        case Reports.Mission:
            label = labels.energy;
            draw_set_color(c_white);
            draw_text(text_x, text_y, label);
            draw_text(text_x + max_label_width + padding, text_y, string(data.total_energy)); text_y += padding;
            
            label = labels.shield_energy;
            draw_set_color(c_white);
            draw_text(text_x, text_y, label);
            draw_text(text_x + max_label_width + padding, text_y, string(data.energy_to_shields)); text_y += padding;
            
            label = labels.available;
            draw_set_color(c_white);
            draw_text(text_x, text_y, label);
            draw_text(text_x + max_label_width + padding, text_y, string(data.available_energy)); text_y += padding;
            
            label = labels.mission_torpedoes;
            draw_set_color(c_white);
            draw_text(text_x, text_y, label);
            draw_text(text_x + max_label_width + padding, text_y, string(data.torpedoes)); text_y += padding;
            
            label = labels.enemies_destroyed;
            draw_set_color(c_white);
            draw_text(text_x, text_y, label);
            draw_text(text_x + max_label_width + padding, text_y, string(data.enemies_destroyed)); text_y += padding;
            
            label = labels.enemies_left;
            draw_set_color(c_white);
            draw_text(text_x, text_y, label);
            draw_text(text_x + max_label_width + padding, text_y, string(data.enemies_left)); text_y += padding;
            
            label = labels.days_left;
            draw_set_color(c_white);
            draw_text(text_x, text_y, label);
            draw_set_color(global.t_colors.yellow);
            draw_text(text_x + max_label_width + padding, text_y, string(data.days_remaining));
			draw_set_color(c_white);
            break;
		case Reports.Scan:
		    ...
		    for (var row = 0; row < 3; row++) {
		        for (var col = 0; col < 3; col++) {
		            var index = row * 3 + col;
		            value = data[index];
		            if (value == "") value = "***"; // Fallback for empty values
		            sx = current_sx + (col - 1); // Map col to dx
		            sy = current_sy + (row - 1); // Map row to dy
		            var e = (value == "***") ? 0 : real(string_char_at(value, 1)); // Extract E
		            var b = (value == "***") ? 0 : real(string_char_at(value, 2)); // Extract B
		            var s = (value == "***") ? 0 : real(string_char_at(value, 3)); // Extract S
    ...
    for (var sy = 0; sy < 8; sy++) {
        for (var sx = 0; sx < 8; sx++) {
            ...
            if (sector.enemynum >= 0) {
                data[idx] = string(sector.enemynum) + string(sector.basenum) + string(sector.starnum);
            }
        }
    }
...
function draw_impulse_grid() {
    if (global.inputmode.mode != InputMode.Impulse) return;
...
function draw_energy_bars() {
    ...
    var shieldsbar = clamp((global.ent.shields / global.game.maxenergy) * 100, 0, 100);
    var energybar  = clamp((global.ent.energy / global.game.maxenergy) * 100, 0, 100);
    var phasersbar = clamp((global.ent.phasers / global.game.maxenergy) * 100, 0, 100);
    ...
    // Draw healthbars
    draw_healthbar(91, 21, 127, 23, shieldsbar, shield_col, shield_col, shield_col, 0, false, false);
    draw_healthbar(142, 21, 178, 23, energybar, energy_col, energy_col, energy_col, 0, false, false);
    draw_healthbar(191, 21, 227, 23, phasersbar, phasers_col, phasers_col, phasers_col, 0, false, false);
...
function draw_briefing() {
    ...
    for (var i = 0; i < array_length(lines); i++) {
        var entry = lines[i];
        var line_text = entry[0] == "format"
            ? lang_format(entry[1], entry[2])
            : lang_get(entry[1]);
...
function draw_gameover_text() {
	if (instance_exists(obj_controller_movies)) {
		...
		else if (global.ent.condition == Condition.Stranded && global.ent.system.warp < 10) {
