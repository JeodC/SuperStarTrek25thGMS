...
function debug_handle_keys() {
    if (!global.debug || global.busy || global.inputmode.mode != InputMode.Bridge) return;
    ...
    for (var i = 0; i < array_length(dir_keys); i++) {
        if (keyboard_check_pressed(dir_keys[i][0])) {
            ...
            change_sector(global.ent.sx + dir_keys[i][1], global.ent.sy + dir_keys[i][2]);
        }
    }
	...
	// Generate particles at a random sector cell
	else if (keyboard_check_pressed(ord("P"))) {
		particle_explosion(irandom_range(0, 7), irandom_range(0, 7));
	}
	// Force enemy attack
...
function debug_draw() {
	if (global.debug) {
		...
		draw_text(xx, yy + 20, "HoverState: " + string(obj_controller_input.hover_state));
...
function debug_galaxy() {   
    // Galaxy contents
    show_debug_message("Galaxy generated -- Enemies: " + string(global.game.totalenemies) + " Bases: " + string(global.game.totalbases) + " Difficulty: " + string(global.difficulty));
    debug_sector();
    for (var sx = 0; sx < 8; sx++) {
        var row = "";
        for (var sy = 0; sy < 8; sy++) {
            var s = global.galaxy[sx][sy];
            row += "[" + string(sx) + "," + string(sy) + "]: " + string(s.enemynum) + string(s.basenum) + string(s.starnum);
            if (sy < 7) row += " ";
        }
...
function debug_print_stats() {
	...
	show_debug_message("Application difficulty: " + string(global.difficulty) + ", Game difficulty: " + string(global.game.difficulty));
	show_debug_message("Game date: " + string(global.game.date), + ", Time left: " + string(daysleft));
	...
	show_debug_message("Current sector: " + string(global.ent.sx) + "," + string(global.ent.sy));
	show_debug_message("Local Enemies: " + string(obj_controller_player.local_enemies));
	show_debug_message("Local Objects: " + string(obj_controller_player.local_objects));
}