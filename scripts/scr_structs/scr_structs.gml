...
function Game() {
	return {
		difficulty: 1, // Struct difficulty separate from global to maintain ongoing session difficulty
		maxenergy: 3000,
		maxtorpedoes: 10,
		enemypower: 0,
		maxstars: 7,
...
function Ship() {
    var ship = {
		...
		/// @description: Moves the player along a path when new local coordinate is chosen
		impulse_move: function(path_array) {
		    if (path_array == undefined || array_length(path_array) < 2) return false;
		...
		/// @description: Smoothly updates the direction and cell position while moving with impulse
		update_impulse_animation: function() {
		    if (!self.animating_impulse) return false;
