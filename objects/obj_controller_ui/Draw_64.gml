...
function draw_title_info() {
	...
	draw_outline(tx, ty, text); draw_set_color(global.t_colors.blue); draw_text_ext(tx, ty, text, spacing, wrap); ty += 10; text = lang_get("title.info2");
	draw_outline(tx, ty, text); draw_set_color(global.t_colors.yellow); draw_text_ext(tx, ty, text, spacing, wrap); ty += 14; text = lang_get("title.info3");
	draw_outline(tx, ty, text); draw_set_color(global.t_colors.blue); draw_text_ext(tx, ty, text, spacing, wrap); ty += 10; text = lang_get("title.info4");
	draw_outline(tx, ty, text); draw_set_color(global.t_colors.yellow); draw_text_ext(tx, ty, text, spacing, wrap); ty += 5; text = lang_format("title.version", { version: global.version });
	
	var version_text = (is_undefined(global.version) || string(global.version) == "") 
    ? "v0.0.0" 
    : string(global.version);
	draw_outline(45, ty, text); draw_set_color(global.t_colors.yellow); draw_text_ext(45, ty, text, spacing, wrap);
