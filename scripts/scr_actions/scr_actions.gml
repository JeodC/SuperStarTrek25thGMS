...
function action_warp(sx, sy) {
  ...
  if (sector.enemynum > 0) {
    ...
    array_push(
        global.queue,
        function() { enemy_attack(obj_controller_player._warpto); });
    return undefined;
  }
    ...
    if (check_valid_move(cx, cy)) {
      ...
      if (path != undefined) {
        if (sector.enemynum < 1) {
          ...
          array_push(
              global.queue,
              function() { enemy_attack(obj_controller_player._path); });
          return true;
        }
      } else {
...
/// @description: Fires a torpedo along the specified angle toward the target
/// @param {real} tx: Target x in cell [0-7]
/// @param {real} ty: Target y in cell [0-7]
...
function action_apply_change(type, value) {
  ...
  case HoverState.Shields:
    ...
    if (shield_change > 0) {
      ...
      queue_dialog(Speaker.Scott, "shields.raised", undefined,
                   {energy : shield_value});
  ...
  case HoverState.Phasers:
    ...
    if (phaser_change > 0) {
      ...
      queue[q_len] = function() { player_phaser_attack(); };
    }
...
function action_setstate(state) {
  ...
  case HoverState.Phasers:
    ...
    var speaker =
        (state == HoverState.Shields) ? Speaker.Scott : Speaker.Chekov;
    var line =
        (state == HoverState.Shields) ? "shields.damaged" : "phasers.damaged";
...
function action_on_screen(status) {
  ...
  case Reports.Mission:
    ...
    data = {
      ...
      days_remaining :
          (global.game.t0 + (global.game.maxdays - global.game.date)),
    };
    break;
  case Reports.Scan:
...
function action_lrs(sx, sy) {
  ...
  if (!is_array(global.galaxy))
    return s;
  if (sx < 0 || sx >= array_length(global.galaxy))
    return s;
  if (!is_array(global.galaxy[sx]))
    return s;
  if (sy < 0 || sy >= array_length(global.galaxy[sx]))
    return s;
